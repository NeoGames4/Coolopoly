...
public class InfoPanel extends UIElement {
	...
	public InfoPanel(Display display) {
		...
		Button settingsButton = new Button((int) (20 + aboutButton.width), 6, Design.INFO_PANEL_HEIGHT - 12, Design.INFO_PANEL_HEIGHT - 12, Button.RELATIVE_TO_BOTTOM_LEFT) {
			@Override
			public void onPressed() {
				display.overlays.add(new SettingsOverlay());
			}
		}.withIcon(Design.getImage("gear-icon.png")).setTooltip("Settings");
		
		Button aiButton = new Button((int) (30 + aboutButton.width + settingsButton.width), 6, Design.INFO_PANEL_HEIGHT - 12, Design.INFO_PANEL_HEIGHT - 12, Button.RELATIVE_TO_BOTTOM_LEFT) {
			@Override
			public void onPressed() {
				final String[] wisdom = {
					"You should definitely buy the next available plot!",
					"Make sure that the servers are well positioned.",
					"BUY BUY BUY!",
					"Be a little more economical.",
					"Make sure that your eyes are at a sufficient distance from the screen.",
					"Stay hydrated.",
					"It's probably better to be frugal right now.",
					"You should invest quickly.",
					"Try to dice something better.",
					"Try to play a little better.",
					"To win, you better get storage fast.",
					"Now would be a good time to buy server.",
					"Next time you roll the dice, try not to land on other players' fields.",
					"Make sure you blink sometimes.",
					"I've seen better players.",
					"Next time, you'd better do the opposite of what you're planning to do.",
					"I have a bad feeling about this.",
					"I have a good feeling about this.",
					"Trust your heart. You're doing a great job.",
					"As Milo once said: Duck.",
					"Try to spend your storage space wisely."
				};
				...
				display.dialogs.add(
					new DialogWindow(
						"Coolopoly AI",
						wisdom[(int) (Math.random()*wisdom.length)],
						options
					)
				);
			}
		}.withIcon(Design.getImage("stars-icon.png")).setTooltip("Coolopoly AI");
		
		Button managePropertiesButton = new Button((int) (40 + aboutButton.width + settingsButton.width + aiButton.width), 6, Design.INFO_PANEL_HEIGHT - 12, Design.INFO_PANEL_HEIGHT - 12, Button.RELATIVE_TO_BOTTOM_LEFT) {
			@Override
			public void onPressed() {
				display.overlays.add(new ManagePropertiesOverlay());
			}
		}.withIcon(Design.getImage("arrow-down-icon.png")).setTooltip("manage(installations)").withButtonStyle(Design.BUTTON_GREEN_STYLE);
		...
		display.uiElements.add(aiButton);
	...
	public void paint(Graphics2D g2, Display display) {
		...
		} else if(gameState != null && gameState.currentPlayer().nameEquals(player) && gameState.toPayEstimate != 0) {
			String diffStr = "-" + gameState.toPayEstimate + " " + Constants.MONEY_UNIT + " (est.)";
			
			g2.setColor(Tools.rgbWithOpaque(
				Design.INFO_PANEL_YELLOW_COLOR,
				(int) lastMoneyOpaqueTransition.getValue()
			));
			g2.setFont(Design.INFO_PANEL_MONEY_CHANGE_FONT);
			g2.drawString(
