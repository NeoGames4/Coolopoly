...
public class DialogWindow extends UIElement {
	...
	public DialogWindow setHotkeyPopable(boolean p) {
		isHotkeyPopable = p;
		return this;
	}
	...
	public void paint(Graphics2D g2, Display display) {
		...
		for(int i = 0; i<lines.size(); i++) {
			...
			if(lineOverflow.length() > 0)
				lines.add(i+1, lineOverflow.substring(0, lineOverflow.length()-1));
		...
		if(isHotkeyPopable) {
			...
			String popNotice = "Press [ESC] to close this dialog window.";
			g2.drawString(popNotice, (ui.getWidth() - g2.getFontMetrics().stringWidth(popNotice))/2, (ui.getHeight() + height)/2 + g2.getFontMetrics().getHeight());
		}
	}
