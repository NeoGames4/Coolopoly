...
public class ManagePropertiesOverlay extends UIOverlay {
	...
	private Transition	opaqueTransition = new Transition(0, 255, Design.PROPERTIES_WINDOW_FADE_DURATION),
	...
	public void pop() {
		pop = true;
		opaqueTransition = new Transition(opaqueTransition.getValue(), 0, Design.PROPERTIES_WINDOW_FADE_DURATION);
	}
	...
	public void paint(Graphics2D g2, Display display) {
		...
		for(int y = 0; y<rows; y++) {
			...
			for(int x = 0; x<columns; x++) {
				...
				// BACKGROUND
				Color backgroundColor = Tools.rgbWithOpaque(Design.PROPERTIES_CARD_BACKGROUND_COLOR, Math.round(opaqueTransition.getValue()));	// TODO mortage color
				...
				String priceStr = canBuildHouse ? "Server (" + (player.properties.containsKey(p.getKey()) ? player.properties.get(p.getKey()) : 0) +"): " + p.getKey().pricePerHouse + " " + Constants.MONEY_UNIT : "No server slots";
				...
				if(updateButtons) {
					Button buyHouse = new Button(cx + cWidth/2 - buySellHouseWidth - Design.PROPERTIES_CONTENT_PADDING/2, fontY += Design.PROPERTIES_CONTENT_PADDING, buySellHouseWidth, buySellHouseHeight) {
						@Override
						public void onPressed() {
					...
					Button mortgageProperty = new Button(mortgageStr, cx + cWidth/2 - mortgageWidth/2, fontY += buySellHouseHeight + Design.PROPERTIES_CONTENT_PADDING) {
						@Override
						public void onPressed() {
							Console.log(mortgageStr);
							// TODO
						}
					}.withButtonStyle(Design.BUTTON_RED_STYLE);
		...
		if(opaqueTransition.isDone() && !pop && System.currentTimeMillis() - latestUpdate > UPDATE_COOLDOWN) {
			...
			if(buttonTransition == null || buttonTransition.to != 1)
				buttonTransition = new Transition(0, 1, Design.PROPERTIES_WINDOW_FADE_DURATION).withEaseDegree(1);
