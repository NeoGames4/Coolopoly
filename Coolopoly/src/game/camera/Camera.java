...
public class Camera {
	...
	/*public boolean debugTransitionTo(CameraState state, long duration) {
		Console.log("NULL CHECK "	+ (transition == null));
		if(transition != null)
			Console.log("IS DONE "		+ transition.isDone());
		Console.log("EQUALS "		+ state.equals(getState()) + "\n");
		if((transition == null || transition.isDone()) && !state.equals(getState())) {
			transition = new CameraTransition(getState(), state, duration);
			return true;
		} return false;
