...
import game.properties.ChangeMoneyEffect;
...
public class GameState {
	...
	public DiceState dice() throws GameIllegalMoveException {
		...
		if(currentPlayer().isInPrison()) {
			if(diceState.isDouble()) {
				currentPlayer().freeFromPrison();
				newNotifications.add(new Notification(currentPlayer().name + " reclaimed sudo permissions and left sandbox isolation!"));
			} else {
				int turnsLeft = currentPlayer().spendPrisonTerm();
				newNotifications.add(new Notification(currentPlayer().name + " is still in sandbox isolation (" + turnsLeft + " left)."));
				return diceState;
			}
		}
	...
	public void buyProperty() throws GameIllegalMoveException {
		...
		if(cPlayer.getMoney() < p.price)
			throw new GameIllegalMoveException("You do not have enough free storage.", Constants.MESSAGE_INTENT_ACTION);
	...
	public void buyHouse(Property prop) throws GameIllegalMoveException {
		...
		if(!streetPropertiesOfPlayerNotMortgaged(prop.street, cPlayer))
			throw new GameIllegalMoveException("You cannot put a server on a (partly) mortgaged street.", Constants.MESSAGE_INTENT_ACTION);
	...
	public void sellHouse(Property prop) throws GameIllegalMoveException {
		...
		cPlayer.changeMoney(prop.pricePerHouse/2, this);
		cPlayer.properties.put(prop, cPlayer.properties.get(prop)-1);
	...
	public void mortgageProperty(Property prop) throws GameIllegalMoveException {
		if(!isRunning())
			throw new GameIllegalMoveException("The game is not running.", Constants.MESSAGE_INTENT_ACTION);		
		...
		if(prop.mortgagePrice < 0)
			throw new GameIllegalMoveException("This property cannot be mortgaged.", Constants.MESSAGE_INTENT_ACTION);
		
		if(!streetPropertiesOfPlayerIsEmpty(prop.street, cPlayer))
			throw new GameIllegalMoveException("You need to sell all servers on this street first.", Constants.MESSAGE_INTENT_ACTION);
		else if(cPlayer.properties.get(prop) < 0)
			throw new GameIllegalMoveException("This property is already mortgaged.", Constants.MESSAGE_INTENT_ACTION);
		
		cPlayer.changeMoney(prop.mortgagePrice, this);
		cPlayer.properties.put(prop, -1);
		
		newNotifications.add(new Notification(currentPlayer().name + " mortgaged " + prop.title + " and received " + prop.mortgagePrice + " " + Constants.MONEY_UNIT + "."));
	}
	
	public void unmortgageProperty(Property prop) throws GameIllegalMoveException {
		if(!isRunning())
			throw new GameIllegalMoveException("The game is not running.", Constants.MESSAGE_INTENT_ACTION);		
		...
		if(cPlayer.properties.get(prop) >= 0)
			throw new GameIllegalMoveException("This property is not mortgaged.", Constants.MESSAGE_INTENT_ACTION);
		
		if(cPlayer.getMoney() < prop.getUnmortgagePrice())
			throw new GameIllegalMoveException("You do not have enough free storage.", Constants.MESSAGE_INTENT_ACTION);
		
		cPlayer.changeMoney(-prop.getUnmortgagePrice(), this);
		cPlayer.properties.put(prop, 0);
		
		newNotifications.add(new Notification(currentPlayer().name + " unmortgaged " + prop.title + " for " + prop.getUnmortgagePrice() + " " + Constants.MONEY_UNIT + "."));
	}
	...
	public boolean streetPropertiesOfPlayerIsEmpty(String street, Player player) {
		for(Property p : Properties.properties) {
			Player oPlayer = getPropertyOwner(p);
			if(street.equals(p.street) && oPlayer != null && oPlayer.nameEquals(player) && oPlayer.properties.get(p) > 0)
				return false;
		} return true;
	}
	
	public boolean streetPropertiesOfPlayerNotMortgaged(String street, Player player) {
		for(Property p : Properties.properties) {
			Player oPlayer = getPropertyOwner(p);
			if(street.equals(p.street) && oPlayer != null && oPlayer.nameEquals(player) && oPlayer.properties.get(p) < 0)
				return false;
	...
	public int toPayEstimate() {
		...
		// PROPERTY EFFECT
		if(prop.effect != null && prop.effect instanceof ChangeMoneyEffect) {
			ChangeMoneyEffect e = (ChangeMoneyEffect) prop.effect;
			if(e.amount < 0)
				toPay += Math.abs(e.amount);
		}
		
		// CARD EFFECT
		if(cardEffect != null && cardEffect.toEarn() < 0)
			toPay += Math.abs(cardEffect.toEarn());
